...
export function ProfileModal() {
  ...
  const handleToggleLinkPreviews = async () => {
    if (!currentUser) return;
    await updateCurrentUser(
      { ...currentUser, link_previews_enabled: !currentUser.link_previews_enabled },
      false
    );
  };
      ...
      <div className="relative w-[350px] h-full bg-[var(--bg-primary)] flex flex-col shadow-2xl animate-slide-in">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 bg-[var(--bg-primary)]">
            ...
            <div className="flex items-center justify-between">
              <span className="text-[var(--text-primary)]">
                {currentUser?.link_previews_enabled ? "On" : "Off"}
              </span>
              <button
                onClick={handleToggleLinkPreviews}
                className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                  currentUser?.link_previews_enabled
                    ? "bg-[var(--accent)]"
            ...
            <p className="text-xs text-[var(--text-secondary)] mt-4">
              Show preview cards for URLs shared in messages.
            </p>
          </div>
        </div>
      </div>
